...
def find_upper_lower(eye_position, class_id,parsing):
    upper_point = None
    lower_point = None
    ...
    eye_mask = (parsing == class_id).astype(np.uint8)
...
def eye_rectangles(parsing, eye_class_id, original_image,height):
...
def evaluate(image_path, cp,height):
    n_classes = 19
    ...
    #print(left_eye_positions,right_eye_positions)
    if len(left_eye_positions)>=2:
        left_eye_position = left_eye_positions[0]
        right_eye_position = left_eye_positions[1]
        #
        # Assuming left_eye_position is a tuple (x, y)
        left_eye_position = (left_eye_position[0] * (height / 1350), left_eye_position[1] * (height / 1350))
        right_eye_position = (right_eye_position[0] * (height / 1350), right_eye_position[1] * (height / 1350))

        #R1,R2=eye_rectangles(parsing, eye_class_id=4,original_image=image,height=250)
        ...
        #R1 = eye_rectangles(parsing, eye_class_id=4,original_image=image,height=250)
        #R2 = eye_rectangles(parsing, eye_class_id=5,original_image=image,height=250)
        ...
        left_eye_position = (left_eye_position[0] * (height / 1350), left_eye_position[1] * (height / 1350))
        right_eye_position = (right_eye_position[0] * (height / 1350), right_eye_position[1] * (height / 1350))
        #R1,R2=eye_rectangles(parsing, eye_class_id=5,original_image=image,height=250)
    else:
    ...
    if chin_point is not None:
        chin_point = (chin_point[0] * (height / 1350), chin_point[1] * (height / 1350))
    else:
        chin_point = None
...
def get_heights_weights_QC(B,T):
...
def eye_open(left_eye,right_eye,parsing,T):
    #upper & lower
    #get upper and lower points for both left and right eyes
    left_upper_eye,left_lower_eye=find_upper_lower(left_eye,class_id=4,parsing=parsing)
    right_upper_eye, right_lower_eye = find_upper_lower(right_eye,class_id=5,parsing=parsing)
...
def face_crops(right_eye,left_eye,eyes_middle_point,A,B,IED):
    ...
    leftward=right_eye[0]/IED
    leftward_crop_QC = round(100 * (sigmoid(leftward, 0.9, 0.1)))
    #print(f"leftward_crop_QC = {leftward_crop_QC}")

    # compute the rightward crop QC of the face in image
    rightward=(A - left_eye[0])/IED
    rightward_crop_QC = round(100 * (sigmoid(rightward, 0.9, 0.1)))
    #print(f"rightward_crop_QC = {rightward_crop_QC}") 

    # compute the downward crop QC of the face in image
    downward = eyes_middle_point[1]/IED
    downward_crop_QC = round(100 * (sigmoid(downward, 1.4, 0.1)))
    #print(f"dwonward_crop_QC = {downward_crop_QC}")  

    # compute the upward crop QC of the face in image
    upward=(B - eyes_middle_point[1])/IED
    upward_crop_QC = round(100 * (sigmoid(upward, 1.8, 0.1)))
...
def get_all_QCs(folder_path):
    ...
    for image_file in image_files:
        ...
        B = height
        A = width
        eye_center,chin_point,left_eye,right_eye,counters,parsing = evaluate(image_file, cp='/home/jing/FIQA_repo/face_parsing_PyTorch/res/cp/79999_iter.pth',height=B)
        print(eye_center)
        if  left_eye is None or right_eye is None or chin_point is None:
            images_path.remove(image_file)
        else:
        #get T
            T = get_T(eye_center,chin_point)
        
            #D,QC = get_heights_weights_QC(B,T)
        ...
        #eye open
            w_eye,eye_openness_QC = eye_open(left_eye,right_eye,parsing,T)
        ...
        #face crops
            leftward,rightward,downward,upward,leftward_crop_QC,rightward_crop_QC,downward_crop_QC,upward_crop_QC = face_crops(right_eye,left_eye,eyes_middle_point,A,B,IED)
        ...
        #eye visible
            ...
            list_A.append(A)
            list_B.append(B)
            list_down_crop_value.append(downward)
            list_up_crop_value.append(upward)
            list_right_crop_value.append(rightward)
            list_left_crop_value.append(leftward)
            list_D.append(D)
            list_T.append(T)
        #head_size QCs:
    ...
    #data={'image paths':images_path, 'leftward crop': list_left_crop_value,'rightward crop': list_right_crop_value, 'downward crop': list_down_crop_value, 'upward crop': list_up_crop_value}
    
    df = pd.DataFrame(data)
    excel_file_path = '/home/jing/FIQA_repo/code/excel outputs/FP_outputs_LFW_crops.csv'
    print(image_file)
